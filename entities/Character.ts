...
const DEFAULT_MODEL_URL = 'https://raw.githubusercontent.com/mrdoob/three.js/master/examples/models/gltf/RobotExpressive/RobotExpressive.glb';
...
export class Character {
  ...
  async load(onLoaded?: (names: string[]) => void, customUrl?: string) {
    const loader = new GLTFLoader();
    const url = customUrl || DEFAULT_MODEL_URL;
    ...
    try {
      ...
      // Try to find an appropriate starting animation
      const startAction = this.actions['Idle'] || this.actions[names[0]];
      if (startAction) {
        startAction.play();
        this.currentAction = startAction.getClip().name;
      }
  ...
  update(delta: number, controls: any, joystick: any, cameraTheta: number, testAnimation?: string | null) {
    ...
    let next = 'Idle';
    if (!this.isGrounded) {
      next = this.actions['Jump'] ? 'Jump' : 'Idle';
    } else if (velMag > 0.5) {
      if (isBoosting && this.actions['Run']) {
        next = 'Run';
      } else if (isBoosting && this.actions['Running']) {
        next = 'Running';
      } else if (velMag > 7.0) {
        next = this.actions['Running'] ? 'Running' : (this.actions['Walk'] ? 'Walk' : (this.actions['Walking'] ? 'Walking' : 'Idle'));
      } else {
        next = this.actions['Walking'] ? 'Walking' : (this.actions['Walk'] ? 'Walk' : 'Idle');
      }
    }
